namespace Social
{
    ...
    public class SocialDataSource
    {
        ...
        public SocialDataSource(string connectionString)
        {
            GetUsers(connectionString);
            GetFriends(connectionString);
        ...
        public UserContext GetUserContext(string userName)
        {
            ...
            List<UserInformation> GetUserFriends(User user)
            {
                ...
                var friends = theFirstFriendCase.Concat(theSecondFriendCase).ToList();

                var friendsCount = friends.Count;

                var output = friendsCount != 0 ? RemoveMistakenlyAddedFriends(friends.Select(user => new UserInformation(user.Name, user.Online, user.UserId)).ToList()) : new List<UserInformation>();
        ...
        private void GetUsers(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                var command = new SqlCommand("SELECT * FROM [Social].[dbo].[Users]", connection);
                ...
                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        var person = new User(
                            ...
                            (bool)dataReader["gender"],
                            (DateTime)dataReader["lastVisit"],
                            (string)dataReader["name"],
                            (bool)dataReader["isOnline"],
        ...
        private void GetMessages(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                var command = new SqlCommand("SELECT * FROM [Social].[dbo].[Messages]", connection);

                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        var authorId = (int)dataReader["authorId"];

                        _messages.Add(new Message(
                            authorId,
                            GetLikes(connectionString, authorId),
                            (int)dataReader["messageId"],
                            (DateTime)dataReader["sendDate"],
                            (string)dataReader["messageText"]));
                    }
                }
            }
        }

        private void GetFriends(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                var command = new SqlCommand("SELECT * FROM [Social].[dbo].[Friends]", connection);
                ...
                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        _friends.Add(new Friend(
                           (int)dataReader["userFrom"],
                           (DateTime)dataReader["sendDate"],
                           (short)dataReader["friendStatus"],
                           (int)dataReader["userTo"]));
                    }
                }
            }
        }

        private List<int> GetLikes(string connectionString, int messageId)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                var command = new SqlCommand("SELECT * FROM [Social].[dbo].[Likes] WHERE messageId = @MessageId", connection);
                ...
                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        likes.Add((int)dataReader["userId"]);
                    }
                }
